...
inter.plot<-function(m, data, var1, var2, seed=324, sims=1000, steps=100){ #"m"equals "m=NULL" as default argument for the model
                    require(arm)
                    set.seed(seed)
                    m2.sims <- sim(m, sims)
                    var12<-paste0(var2,":",var1)  #paste take strings and make it to a longer string
                    coef.dem <- data.frame(fake= seq(min(m$model[var2], na.rm=T), 
                                         max(m$model[var2], na.rm=T),
                                         length.out=steps), 
                       coef1 = NA, ub = NA, lb = NA) 

for(i in 1:100) {   
    coef$coef[i] <- mean(m.sims@coef[var1] + coef$fake[i]*m.sims@coef[var12])
    coef$ub[i] <- quantile(m.sims@coef[var1] + coef$fake[i]*m.sims@coef[var12], .975, na.rm=T)
    coef$lb[i] <- quantile(m.sims@coef[var1] + coef$fake[i]*m.sims@coef[var12], .025, na.rm=T)    
}   #so if there is another interaction, just plus the coefficient of that one in. Do partial deriviative by hand to ensure which coefficient is added.
