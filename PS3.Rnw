...
#Check the data, which do not include any irrelevant values that need to be manipulated before the MCMC
list<-subset(mex,select=c(1, 6:13))
lapply(list, table)  
##Get a general sense of the relationships between Y and Xs. Because of using a using default regression prior, the result of MCMC should be similar as the results given by the classical regression.
...
m1.mc.2<-MCMCregress(PRIfeel~rightide+econpers+econnat+female+ses+churchatt, 
            data = mex, burnin = 1000, mcmc = 10000,
            thin = 1, verbose = 0, seed = 3182, 
             b0 = 0, B0 = 0.1, c0 = 2, d0 = 5.544,   # a weakly informative prior, a mean equal to the marginal variance of PRIfeel
            marginal.likelihood = "Chib95")
summary(m1.mc.2)
...
@
As seen in the result, the conditional variable of attitude towards national economy has a significant effect on the ideology variable.
